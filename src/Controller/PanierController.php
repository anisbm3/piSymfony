...
public function new(Request $request, EntityManagerInterface $entityManager): Response
{
   ...
   $entityManager->flush();
    ...
    public function Payer(Request $request,int $panierId, PanierRepository $panierRepository): Response
    {
        $id = $request->query->get('idpanier'); // Récupérer la valeur du paramètre de requête 'total'
        $produits = $panierRepository->findProdDetailsByPanierId($panierId);
        $totalPrice = 0;
